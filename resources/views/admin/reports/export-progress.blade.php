...
<section class="page-heading">
  <div>
    <p class="eyebrow">Export Laporan Kinerja Bulanan</p>
...
<section class="export-progress-section">
  <article class="card" style="padding: 2rem; max-width: 600px; margin: 2rem auto; text-align: center;">
    ...
    <div id="export-done" style="display: none;">
      ...
      <br><br>
      <a href="{{ route('dashboard') }}" class="ghost-action">Kembali ke Dashboard</a>
    </div>
    ...
    <div id="export-error" style="display: none;">
      ...
      <a href="{{ route('dashboard') }}" class="primary-action">Kembali ke Dashboard</a>
    </div>
  </article>
</section>
@endsection
...
(function() {
  ...
  function processStep() {
    ...
    fetch('/admin/report-jobs-perf/' + reportJobId + '/step', {
      method: 'POST',
      headers: {
    ...
    .then(data => {
      if (data.status === 'completed') {
        ...
        showDone('/admin/report-jobs-perf/' + reportJobId + '/download');
        return;
      }
  ...
  function startExport() {
    ...
    .then(res => {
      if (!res.ok) {
        return res.json().then(d => { throw new Error(d.message || 'Gagal memulai'); });
      }
      return res.json();
    })
    .then(data => {
